<template>
          ...
          <div style="width: 100%">
                  ...
                  <el-card class="card">
                    ...
                    >
                      ...
                      <el-table-column label="操作" #default="{ row }">
                        <el-button type="primary" size="small" @click="deletePerson(row.id)">删除</el-button>
                      </el-table-column>
                    </el-table>
                  </el-card>
                </div>
                <div style="width: 100%">
                  ...
                  <el-card class="card">
                    <el-table :data="form.equipment" :key="tableKey2" style="width: 100%">
                      ...
                      <!-- 新增的图片列 -->
                      <el-table-column prop="name" label="图片" style="width: 20%">
                        <template #default="{ row }">
                          <img
                            v-if="row.name === '正压防护服'"
                      ...
                      <el-table-column prop="name" label="操作" style="width: 20%">
                        <template #default="{ row }">
                      ...
                      <el-table-column label="操作" style="width: 10%">
                        <template #default="{ row }">
                          <el-button type="success" size="small" @click="deleteEquiement(row.id)">删除</el-button>
                        </template>
                      </el-table-column>
                    </el-table>
                  </el-card>
            ...
            <el-dialog title="智能对比" v-model="uploadPic" width="80%" :before-close="handleClosePic">
              ...
              <el-upload
                ...
                style="margin-left: 5%; margin-bottom: 20px"
                class="upload-demo"
                ref="uploadInput12"
                action="https://jsonplaceholder.typicode.com/posts/"
                :on-remove="handleRemove"
              ...
              >
                ...
                <div v-if="compareImages" style="align-items: center; margin-left: 10px">
                  <el-image
                    :src="nuejiImage"
                    style="width: 600px; height: 400px; object-fit: cover"
                  ></el-image>
              ...
              <template #footer>
                <span>
                  <el-button @click="compare">对 比</el-button>
                  <el-button type="primary" @click="handleClosePic">确 定</el-button>
                </span>
              </template>
            </el-dialog>
  ...
  <el-dialog title="输入案件id" v-model="isJump" :before-close="handleClose" width="300">
...
const deletePerson = (disposalId) => {
  ElMessageBox.confirm("是否删除该人员", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(() => {
    form.person = form.person.filter((person) => person.id !== disposalId);
    personIdList.value = personIdList.value.filter(
      (person) => person !== disposalId
    );
  });
};

const deleteEquiement = (disposalId) => {
  ElMessageBox.confirm("是否删除该设备", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(() => {
    form.equipment = form.equipment.filter(
      (equipment) => equipment.id !== disposalId
    );
    EquipmentIdList.value = EquipmentIdList.value.filter(
      (equipment) => equipment !== disposalId
    );
  });
};
...
//   if (personID.value == undefined) {
//     ElMessage.error("请输入警务号");
//     return;
//   }
//   post(
//     "/api/risk/select_personById",
//     {
//       id: personID.value,
//     },
//     (data) => {
//       tempPersons.value = persons.value;
//       persons.value = data;
...
// // 监听 personID 的变化
// watch(personID, (newVal, oldVal) => {
//   if (newVal === "") {
//     // 如果 personID 为空，执行某些操作
//     SearchAll();
//   }
// });
...
const SearchAll = () => {
  if (personID.value == undefined) {
    ...
    //   });
    // });
    console.log("personID.value is undefined");
  }
};
