                  ...
                  <el-row>
                    ...
                    <el-col :span="6">
                      <el-popover placement="right" :width="400" trigger="click">
                        ...
                        <img src="./image/IQA.png" alt="Image" style="width: 550px; height: auto;display: block;" />
                      </el-popover>
                    </el-col>
                    <el-col :span="6">
                      <el-popover placement="right" :width="400" trigger="click">
...
function createInvest() {
  ...
  const requestBody = {
    ...
    windSpeed:form.value.windpower,
...
const queryInvest = async () => {
  console.log("执行查询并显示invest");

  try {
    // 发送 GET 请求获取数据
    const response = await fetch(axios.defaults.baseURL + "/api/invest/queryInvest?id=" + id, {
      method: "GET",
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    form.value.activity = data.message.gather;

    form.value.temperature = data.message.temperature;
    form.value.weather = data.message.weather;
